...
diamonds %>%
  ...
  mutate(old_price=price*runif(n = 7,min=-.2,max = .5)) %>%
tile_matrix(values = price,text = color,former=old_price,target=5000,roundVal = 0)
```
...
```{r}
suppressPackageStartupMessages(library(dplyr))
df <- data_frame(v=seq(from=0,to=100,by=10),
                 t="comparison to 50",
                 f=50)

## Let's pretend that all previous values were 50... so:
df %>% tile_matrix(values = v,text = t)
```
