...
test_that("classes work", {
  ...
  div_maker(subtitle = "Quantativity factors",textModifier = "h1",
                     ...
                     solo_box(value = 34)) %>% class %>%
    expect_equal(. , "shiny.tag")

  finisher(title = "Hello",
           solo_box(value = 3.57, txt = "Times apple eaten",
                    icon = "apple")) %>% class %>%
    expect_equal(. , "shiny.tag")
...
test_that("all colors work",{
  expect_equal(solo_gradient_box(value = 10, txt = "blah",former = 5) %>%
                 unlist %>% grepl("danger", x = .) %>%
                 sum, 2)
  expect_equal(solo_gradient_box(value = 80, txt = "blah",former = 5) %>%
                 unlist %>% grepl("warning", x = .) %>%
                 sum, 2)
  expect_equal(solo_gradient_box(value = 95, txt = "blah",former = 5) %>%
                 unlist %>% grepl("success", x = .) %>%
                 sum, 2)
})

test_that("errors error out", {
  expect_error(tile_matrix(data = head(iris),
              values = Species,
              txt = Species))
})

test_that("protections work", {
  multi_box(values = c(3,45), number_zoom = 300, icons = c("apple","calendar"),
            type = "warning",
            txt = c("times","reports")) %>%
    class %>%
    expect_equal(. , "shiny.tag")
